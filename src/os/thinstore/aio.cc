...
std::ostream& operator<<(std::ostream& os, const th_aio_t& aio)
{
  unsigned i = 0;
  os << "aio: ";
...
int th_aio_queue_t::submit_batch(aio_iter begin, aio_iter end, 
			      ...
			      int *retries)
{
  ...
  aio_iter cur = begin;
  struct th_aio_t *piocb[aios_size];
  int left = 0;
  while (cur != end) {
    cur->priv = priv;
    *(piocb+left) = &(*cur);
...
int th_aio_queue_t::get_next_completed(int timeout_ms, th_aio_t **paio, int max)
{
#if defined(HAVE_LIBAIO)
  io_event events[max];
#elif defined(HAVE_POSIXAIO)
...
int th_aio_queue_t::min_get_next_completed(int timeout_ms, th_aio_t **paio, int max, int min)
{
#if defined(HAVE_LIBAIO)
  io_event events[max];
#elif defined(HAVE_POSIXAIO)
  ...
  do {
#if defined(HAVE_LIBAIO)
    r = io_getevents(ctx, min, max, events, &t);
#elif defined(HAVE_POSIXAIO)
    // do not support !!!
    assert(0);
    r = kevent(ctx, NULL, 0, events, max, &t);
  ...
  for (int i=0; i<r; ++i) {
    ...
    paio[i]->rval = events[i].res;
#else
    paio[i] = (th_aio_t*)events[i].udata;
    if (paio[i]->n_aiocb == 1) {
      paio[i]->rval = aio_return(&paio[i]->aio.aiocb);
    } else {
      // Emulate the return value of pwritev.  I can't find any documentation
