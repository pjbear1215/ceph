...
ThKernelDevice::ThKernelDevice(CephContext* cct, aio_callback_t cb, void *cbpriv, aio_callback_t d_cb, void *d_cbpriv)
  : ThBlockDevice(cct, cb, cbpriv),
    aio(false), dio(false),
...
int ThKernelDevice::open(const string& p)
{
  path = p;
  int r = 0, i = 0;
  ...
  if (0 > r) {
    dout(0) << __func__ << " omw fail !!!!! r " << -errno << dendl;
  }
  ...
  // thinstore : temporaly disable lock
#if 0
  if (lock_exclusive) {
    r = _lock();
    if (r < 0) {
...
int ThKernelDevice::get_devices(std::set<std::string> *ls) const
{
  if (devname.empty()) {
    return 0;
  }
...
#if 0
int ThKernelDevice::collect_metadata(const string& prefix, map<string,string> *pm) const
{
  (*pm)[prefix + "support_discard"] = stringify((int)(bool)support_discard);
  (*pm)[prefix + "rotational"] = stringify((int)(bool)rotational);
  (*pm)[prefix + "size"] = stringify(get_size());
  (*pm)[prefix + "block_size"] = stringify(get_block_size());
  (*pm)[prefix + "driver"] = "ThKernelDevice";
  if (rotational) {
    (*pm)[prefix + "type"] = "hdd";
  } else {
    (*pm)[prefix + "type"] = "ssd";
...
void ThKernelDevice::_detect_vdo()
{
...
bool ThKernelDevice::get_thin_utilization(uint64_t *total, uint64_t *avail) const
{
  if (vdo_fd < 0) {
    return false;
  }
...
int ThKernelDevice::choose_fd(bool buffered, int write_hint) const
{
...
int ThKernelDevice::_discard_start()
{
    discard_thread.create("bstore_discard");
    return 0;
}

void ThKernelDevice::_discard_stop()
{
  dout(10) << __func__ << dendl;
  {
...
void ThKernelDevice::discard_drain()
{
  dout(10) << __func__ << dendl;
  std::unique_lock l(discard_lock);
...
#if 0
void ThKernelDevice::_aio_thread_th()
{
  dout(10) << __func__ << " start" << dendl;
  ...
  while (!aio_stop) {
    ...
    th_aio_t *aio[max];
    int r = aio_queue[sd_index].get_next_completed(cct->_conf->bdev_aio_poll_ms,
					 aio, max);
    if (r < 0) {
    ...
    if (r > 0) {
      ...
      for (int i = 0; i < r; ++i) {
	ThIOContext *ioc = static_cast<ThIOContext*>(aio[i]->priv);
	dout(0) << __func__ << " got " << i << " aios" << dendl;
...
void ThKernelDevice::_aio_thread()
{
  dout(10) << __func__ << " start" << dendl;
  int inject_crash_count = 0;
  while (!aio_stop) {
...
void ThKernelDevice::_discard_thread()
{
  std::unique_lock l(discard_lock);
...
int ThKernelDevice::queue_discard(interval_set<uint64_t> &to_release)
{
  if (!support_discard)
    return -1;
...
void ThKernelDevice::_aio_log_start(
  ThIOContext *ioc,
  uint64_t offset,
  uint64_t length)
{
...
void ThKernelDevice::debug_aio_link(th_aio_t& aio)
{
...
void ThKernelDevice::debug_aio_unlink(th_aio_t& aio)
{
...
void ThKernelDevice::_aio_log_finish(
  ThIOContext *ioc,
  uint64_t offset,
  uint64_t length)
{
...
void ThKernelDevice::aio_submit(ThIOContext *ioc)
{
  ...
  // wal aio's.
  list<th_aio_t>::iterator e = ioc->running_aios.begin();
  ...
  if (cct->_conf->bdev_debug_aio) {
    list<th_aio_t>::iterator p = ioc->running_aios.begin();
    while (p != e) {
  ...
  int r, retries = 0;
#if 0
  dout(0) << __func__ << " pending to write " << pending 
	  << " running_aios " << ioc->running_aios.size() << dendl;
#endif
...
int ThKernelDevice::_sync_write(uint64_t off, bufferlist &bl, bool buffered, int write_hint)
{
  uint64_t len = bl.length();
  // selective dispatch
  dout(30) << __func__ << " 0x" << std::hex << off << "~" << len
  ...
  if (r < 0) {
    r = -errno;
    dout(0) << __func__ << " off " << off << " len " << bl.length() << dendl;
  ...
  int write_hint)
{
  ...
  dout(20) << __func__ << " 0x" << std::hex << off << "~" << len << std::dec
	   ...
	   << dendl;
  // Need a check : Do we really need this?
...
{
  uint64_t len = bl.length();
  dout(0) << __func__ << " 0x" << std::hex << off << "~" << len << std::dec
	   << (buffered ? " (buffered)" : " (direct)")
	   << dendl;
  // Need a check : Do we really need this?
  //ceph_assert(is_valid_io(off, len));
  ...
  *_dout << dendl;
  ...
  if (aio && dio && !buffered) {
	...
	rand() % cct->_conf->bdev_inject_crash == 0) {
      ...
      ioc->pending_aios.push_back(th_aio_t(ioc, choose_fd(false, write_hint)));
      ++ioc->num_pending;
...
int ThKernelDevice::discard(uint64_t offset, uint64_t len)
{
  int r = 0;
  if (support_discard) {
      dout(10) << __func__
...
int ThKernelDevice::read(uint64_t off, uint64_t len, bufferlist *pbl,
		      ThIOContext *ioc,
		      bool buffered)
{
  ...
  ThIOContext *ioc)
{
  ...
  if (aio && dio) {
    ceph_assert(is_valid_io(off, len));
    _aio_log_start(ioc, off, len);
    ioc->pending_aios.push_back(th_aio_t(ioc, fd_directs[WRITE_LIFE_NOT_SET]));
    ++ioc->num_pending;
    th_aio_t& aio = ioc->pending_aios.back();
    bufferptr p = buffer::create_small_page_aligned(len);
    aio.bl.append(std::move(p));
    aio.bl.prepare_iov(&aio.iov);
    aio.preadv(off, len);
...
int ThKernelDevice::direct_read_unaligned(uint64_t off, uint64_t len, char *buf)
{
...
int ThKernelDevice::read_random(uint64_t off, uint64_t len, char *buf,
                       bool buffered)
{
  dout(5) << __func__ << " 0x" << std::hex << off << "~" << len << std::dec
          << "buffered " << buffered
...
int ThKernelDevice::invalidate_cache(uint64_t off, uint64_t len)
{
  dout(5) << __func__ << " 0x" << std::hex << off << "~" << len << std::dec
	  << dendl;
...
#if 0
int ThKernelDevice::_wait_all_completed(int nums)
{
  int max = cct->_conf->bdev_aio_reap_max;
  th_aio_t *aio[max];
  int r = aio_queue.min_get_next_completed(cct->_conf->bdev_aio_poll_ms,
				       aio, max, nums);
  assert(nums == r);
  if (r < 0) {
    derr << __func__ << " got " << cpp_strerror(r) << dendl;
  ...
  if (r > 0) {
    dout(0) << __func__ << " got " << r << " completed aios" << dendl;
    for (int i = 0; i < r; ++i) {
      ThIOContext *ioc = static_cast<IOContext*>(aio[i]->priv);
      long r = aio[i]->get_return_value();
      if (r < 0) {
	derr << __func__ << " got r=" << r << " (" << cpp_strerror(r) << ")"
	     << dendl;
