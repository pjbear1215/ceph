...
struct th_aio_t {
  ...
  th_aio_t(void *p, int f) : priv(p), fd(f), offset(0), length(0), rval(-1000) {
  }
...
typedef boost::intrusive::list<
  ...
  boost::intrusive::member_hook<
    ...
    &th_aio_t::queue_item> > th_aio_list_t;

struct th_aio_queue_t {
  int max_iodepth;
#if defined(HAVE_LIBAIO)
  io_context_t ctx;
#elif defined(HAVE_POSIXAIO)
  ...
  typedef list<th_aio_t>::iterator aio_iter;

  explicit th_aio_queue_t(unsigned max_iodepth)
  ...
  int get_next_completed(int timeout_ms, th_aio_t **paio, int max);
  int min_get_next_completed(int timeout_ms, th_aio_t **paio, int max, int min);
};
