...
#ifndef CEPH_OS_THINSTORE_KERNELDEVICE_H
#define CEPH_OS_THINSTORE_KERNELDEVICE_H
...
class ThKernelDevice : public ThBlockDevice {
  ...
  struct AioCompletionThread : public Thread {
    ThKernelDevice *bdev;
    int sd_index;
    explicit AioCompletionThread(ThKernelDevice *b, int cpu = -1) : bdev(b), sd_index(cpu) {}
    void *entry() override {
#if 0
      if (sd_index != -1) {
	cpu_set_t set;
  ...
  struct DiscardThread : public Thread {
    ThKernelDevice *bdev;
    explicit DiscardThread(ThKernelDevice *b) : bdev(b) {}
    void *entry() override {
  ...
  void _aio_log_start(ThIOContext *ioc, uint64_t offset, uint64_t length);
  void _aio_log_finish(ThIOContext *ioc, uint64_t offset, uint64_t length);
  ...
  th_aio_list_t debug_queue;
  ceph::mutex debug_queue_lock = ceph::make_mutex("KernelDevice::debug_queue_lock");
  th_aio_t *debug_oldest = nullptr;
  utime_t debug_stall_since;
  void debug_aio_link(th_aio_t& aio);
  void debug_aio_unlink(th_aio_t& aio);

  void _detect_vdo();
  int choose_fd(bool buffered, int write_hint) const;

public:
  ThKernelDevice(CephContext* cct, aio_callback_t cb, void *cbpriv, aio_callback_t d_cb, void *d_cbpriv);

  void aio_submit(ThIOContext *ioc) override;
