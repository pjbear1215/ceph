...
#ifndef CEPH_OS_THINSTORE_BLOCKDEVICE_H
#define CEPH_OS_THINSTORE_BLOCKDEVICE_H
...
public:
...
#if defined(HAVE_LIBAIO) || defined(HAVE_POSIXAIO)
  std::list<th_aio_t> pending_aios;    ///< not yet submitted
  std::list<th_aio_t> running_aios;    ///< submitting or submitted
#endif
  std::atomic_int num_pending = {0};
  std::atomic_int num_running = {0};
  bool allow_eio;
  //vector<uint32_t> alloc_data_block_no;
  bool batch_direct_io = false;
  int batch_io_index = 0;
  int batch_io_size = 0;
  ...
  // no copying
  ThIOContext(const ThIOContext& other) = delete;
  ThIOContext &operator=(const ThIOContext& other) = delete;
...
private:
  ceph::mutex ioc_reap_lock = ceph::make_mutex("ThBlockDevice::ioc_reap_lock");
  std::vector<ThIOContext*> ioc_reap_queue;
...
public:
  ...
  ThBlockDevice(CephContext* cct, aio_callback_t cb, void *cbpriv)
  : cct(cct),
    size(0),
    block_size(0),
    aio_callback(cb),
 ...
 {}
  ...
  void queue_reap_ioc(ThIOContext *ioc);
  void reap_ioc();
  ...
  //virtual int _wait_all_completed(int) = 0;
  friend class ThinStore;
...
#endif //CEPH_OS_THINSTORE_BLOCKDEVICE_H
