				    ...
				    uint64_t start_offset, bool block)
{
  ...
  for (;iter != manifest.chunk_map.end(); ++iter) {
	...
	if (ssc && ssc->snapset.clones.size() > 0) {
	  vector<snapid_t>::reverse_iterator p = ssc->snapset.clones.rbegin();
	  while (p != ssc->snapset.clones.rend() && *p >= soid.snap)
	    ++p;
	  if (p != ssc->snapset.clones.rend()) {
	    hobject_t next = soid;
	    next.snap = *p;
	    ceph_assert(next.snap < soid.snap);
	    if (check_has_ref_snap(soid, *p, iter->second)) {
	      has_reference = true;
	    }
	  }
	} 
