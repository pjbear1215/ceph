...
#endif
  comp_worker(cct->_conf->osd_completion_worker)
{
  objecter->init();
}
...
public:
  ...
  virtual OpQueueItem::OrderLocker::Ref get_order_locker() override final {
    ...
    public:
      ...
      virtual void lock_suspend_timeout(
	ThreadPool::TPHandle &handle) override final {
	pg->ops_sequence_lock(handle);
	return;
      }
      virtual void unlock() override final {
	pg->ops_sequence_unlock();
	return;
      }
    };
    return OpQueueItem::OrderLocker::Ref(
...
{
  assert(cb);
  comp_worker.reserve_lazy_completion(pg->get_pgid().ps(), cb, second);
}
